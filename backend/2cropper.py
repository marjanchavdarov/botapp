...
"""
    ...
    # Inject few-shot examples into prompt if available
    if examples:
        example_parts = []
        for ex in examples[:3]:
            boxes_str = json.dumps(ex.get("boxes", []), ensure_ascii=False)
            example_parts.append(f"Example page (page {ex.get('page_number','?')}):\nCorrect boxes: {boxes_str}")
        if example_parts:
            examples_text = "\n\nHere are examples of CORRECT bounding boxes from the same store to guide you:\n" + "\n---\n".join(example_parts) + "\n\nNow detect products in the NEW image using the same style."
            # inject into body parts
            body["contents"][0]["parts"].insert(0, {"text": examples_text})
    body = {
        "contents": [{"parts": [
    ...
    try:
        ...
        for page_num, page_data in sorted(pages.items()):
            ...
            try:
                ...
                # Fetch few-shot examples for this store
